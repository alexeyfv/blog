...
Частичная реализация функций в F# может быть использована для создания результата, эквивалентного внедрению зависимостей. Такой код компилируется и работает как предполагалось, но не является функциональным. Причина этого в том, что (большинство) зависимостей по своей природе не являются чистыми. Они либо недетерминированы, либо имеют сайд-эффекты или всё сразу, и это часто ключевая причина того, что они в первую очередь учитываются как зависимости.
...
Функциональное программирование решает проблему разделения сайд-эффектов от бизнес логики другим путём. Это тема следующей статьи.
