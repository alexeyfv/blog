...
Это перевод первой статьи Марка Симана из серии ["От внедрения зависимостей к отказу от зависимостей"](https://blog.ploeh.dk/2017/01/27/from-dependency-injection-to-dependency-rejection/).
...
## Черепахи на всём пути вниз

> Заголовок — это отсылка к [проблеме бесконечного регресса](https://en.wikipedia.org/wiki/Turtles_all_the_way_down)

Можно возразить, что метод `Post` — это частный случай, т.к. он является частью API и поэтому должен следовать определённым правилам, а поскольку эти правила не применяются далее в коде, то можно реализовать остальные объекты просто передавая зависимости как аргументы.
...
Этот метод содержит другую зависимость: `reservationsRepository`. Откуда она появилась? Можно ли сделать метод `TryAccept` таким, чтобы он принимал `reservationsRepository` как аргумент?
...
Этот конструктор принимает не только реализацию `IReservationsRepository`, но также и целочисленное значение, представляющее вместимость рассматриваемого ресторана. Это показывает то, что [зависимости могут быть также и примитивными типами](https://blog.ploeh.dk/2012/07/02/PrimitiveDependencies).

### Резюме

Внедрение зависимостей — это, в некотором смысле, просто определенный способ для объектов принимать аргументы. Однако часто объекты имеют роли, определенные реализуемыми ими интерфейсами. Таким объектам могут потребоваться зависимости, которые недоступны через API, определенные этими интерфейсами, поэтому вам придется внедрять их через члены, принадлежащие конкретному рассматриваемому классу. Внедрение зависимостей через конструктор класса — лучший способ сделать это.
