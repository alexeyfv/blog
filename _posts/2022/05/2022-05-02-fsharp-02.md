...
- Для методов `hello3` и `hello4` явно указан тип `x`. 
- Тела методов `hello2`, `hello3` и `hello4` выглядят довольно объёмно. Если, например `hello3`, написать на C#, то скомпилированный код займёт меньше места:
